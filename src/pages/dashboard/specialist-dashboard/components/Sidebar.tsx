import { useAuth } from '../../../../shared/auth/useAuth'

export default function Sidebar() {
  const { logout } = useAuth()
  const path = window.location.pathname.toLowerCase()
  const isDashboard = path === '/dashboard/specialist'
  const isPortfolio = path === '/dashboard/specialist/portfolio'
  const isProfile = path === '/dashboard/specialist/profile'

  return (
    <aside className="sidebar">
      <div className="sidebar-logo">
        <svg
          width="109"
          height="36"
          viewBox="0 0 109 36"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          aria-hidden="true"
        >
          <path
            d="M17.5486 0.760151C17.5486 0.27147 17.9787 -0.107923 18.4437 0.0278454C18.6737 0.09499 18.9009 0.175472 19.124 0.269284L31.3641 7.13849C32.3706 8.10711 33.0418 9.38802 33.2626 10.7863V25.0234C33.0417 26.4214 32.3705 27.7019 31.3642 28.6704L19.1229 35.5402C18.9001 35.6338 18.6734 35.7141 18.4437 35.7812C17.9787 35.9169 17.5486 35.5375 17.5486 35.0489V0.760151Z"
            fill="#5260FF"
          />
          <path
            d="M15.5544 34.9742C15.5544 35.4753 15.1035 35.8572 14.6331 35.6973C14.2966 35.5829 13.9674 35.4393 13.6491 35.2666L12.8282 34.821L10.9169 33.6974C9.89124 33.1332 9.2529 32.0483 9.2529 30.8692V4.93965C9.25292 3.76055 9.89122 2.67561 10.9169 2.11145L12.8229 0.990959L13.6491 0.542605C13.9674 0.369818 14.2967 0.226177 14.6332 0.111687C15.1035 -0.0483587 15.5544 0.33356 15.5544 0.834595V34.9742Z"
            fill="#5260FF"
          />
          <path
            d="M8.29571 30.9319C8.29571 31.5365 7.66018 31.9254 7.12913 31.6458L5.17376 30.6162C4.14805 30.052 3.50972 28.806 3.50972 27.6269V8.19073C3.50974 7.01162 4.14804 5.76566 5.17376 5.2015L7.12916 4.17198C7.66021 3.89238 8.29571 4.28129 8.29571 4.88589V30.9319Z"
            fill="#5260FF"
          />
          <path
            d="M2.71206 27.574C2.71206 28.2251 2.01046 28.5668 1.59863 28.0656C0.649982 26.9111 4.66563e-06 25.3533 0 24.0082V11.801C6.5227e-08 10.2947 0.605057 8.81955 1.55989 7.66527C1.98097 7.15623 2.71206 7.50465 2.71206 8.16782V27.574Z"
            fill="#5260FF"
          />
          <path
            d="M91.5547 27.4791L98.1227 8.33057H101.667L108.261 27.4791H105.212L103.935 23.639H95.8813L94.6041 27.4791H91.5547ZM96.6892 21.105H103.127L100.234 12.6148L99.9211 11.5438L99.5823 12.6148L96.6892 21.105Z"
            fill="#0B1215"
          />
          <path
            d="M93.2248 8.33057V11.1258H87.4486V27.4791H84.3504V11.1258H78.5742V8.33057H93.2248Z"
            fill="#0B1215"
          />
          <path
            d="M68.6677 10.8896C65.0008 10.8896 61.7239 13.5493 61.7239 17.9821C61.7239 22.2324 64.7407 25.0224 68.6677 25.0224C72.6207 25.0224 75.6375 22.2324 75.6375 17.9821C75.6375 13.5493 72.3607 10.8896 68.6677 10.8896ZM58.7852 17.956C58.7852 12.402 62.7902 8.07349 68.6677 8.07349C74.5452 8.07349 78.5763 12.402 78.5763 17.956C78.5763 23.51 74.8573 27.8646 68.6677 27.8646C62.5041 27.8646 58.7852 23.51 58.7852 17.956Z"
            fill="#0B1215"
          />
          <path
            d="M51.0903 27.8645C47.3871 27.8645 44.892 25.7124 44.2617 22.983L46.9406 21.8282C47.4922 23.9015 48.9367 25.1088 51.0903 25.1088C53.2702 25.1088 54.452 24.164 54.452 22.3793C54.452 20.6472 53.2439 19.5974 50.4074 18.5476C46.6255 17.1567 44.8395 15.6082 44.8395 12.8263C44.8395 10.0969 47.0982 7.94482 50.6963 7.94482C53.5328 7.94482 55.5814 8.96836 56.9208 11.2254L54.5571 12.6951C53.6641 11.2516 52.4297 10.6218 50.7751 10.6218C49.0417 10.6218 47.7811 11.4354 47.7811 12.9051C47.7811 14.4272 49.3569 15.3196 51.8782 16.2644C55.6864 17.7078 57.4986 19.7286 57.4986 22.4843C57.4986 25.345 55.1874 27.8645 51.0903 27.8645Z"
            fill="#0B1215"
          />
        </svg>
      </div>

      <nav className="sidebar-nav">
        <div className={`dash-item-wrap${isDashboard ? ' is-active' : ''}`}>
          <a className={`dash-item${isDashboard ? ' is-active' : ''}`} href="/dashboard/specialist">
            <span className="dash-icon">
              <svg
                width="25"
                height="25"
                viewBox="0 0 25 25"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M21.46 14.1875H16.6038C15.9638 14.1901 15.3509 14.4456 14.8986 14.8982C14.4462 15.3508 14.1911 15.9638 14.1888 16.6038V21.46C14.1884 21.7772 14.2507 22.0914 14.3719 22.3846C14.4932 22.6777 14.671 22.9441 14.8954 23.1684C15.1197 23.3927 15.386 23.5706 15.6792 23.6918C15.9723 23.8131 16.2865 23.8753 16.6038 23.875H21.46C22.1004 23.8747 22.7145 23.6201 23.1673 23.1673C23.6201 22.7145 23.8747 22.1004 23.875 21.46V16.6038C23.8753 16.2865 23.8131 15.9723 23.6918 15.6792C23.5706 15.386 23.3927 15.1197 23.1684 14.8954C22.9441 14.671 22.6777 14.4932 22.3846 14.3719C22.0914 14.2507 21.7772 14.1884 21.46 14.1888M21.46 0.750001H3.165C2.84777 0.749672 2.53358 0.811914 2.24043 0.933162C1.94728 1.05441 1.68093 1.23229 1.45661 1.4566C1.23229 1.68092 1.05441 1.94728 0.933164 2.24043C0.811915 2.53358 0.749674 2.84777 0.750003 3.165V8.02125C0.749509 8.33859 0.811629 8.65291 0.932804 8.9462C1.05398 9.2395 1.23183 9.50601 1.45616 9.73046C1.6805 9.95491 1.94691 10.1329 2.24014 10.2542C2.53338 10.3755 2.84766 10.4378 3.165 10.4375H21.46C21.7772 10.4378 22.0914 10.3756 22.3846 10.2543C22.6777 10.1331 22.9441 9.95522 23.1684 9.7309C23.3927 9.50658 23.5706 9.24022 23.6918 8.94707C23.8131 8.65392 23.8753 8.33974 23.875 8.0225V3.165C23.8753 2.84777 23.8131 2.53358 23.6918 2.24043C23.5706 1.94728 23.3927 1.68092 23.1684 1.4566C22.9441 1.23229 22.6777 1.05441 22.3846 0.933162C22.0914 0.811914 21.7772 0.749672 21.46 0.750001ZM8.02125 14.1875H3.165C2.84766 14.1872 2.53338 14.2495 2.24014 14.3708C1.94691 14.4921 1.6805 14.6701 1.45616 14.8945C1.23183 15.119 1.05398 15.3855 0.932804 15.6788C0.811629 15.9721 0.749509 16.2864 0.750003 16.6038V21.46C0.749674 21.7772 0.811915 22.0914 0.933164 22.3846C1.05441 22.6777 1.23229 22.9441 1.45661 23.1684C1.68093 23.3927 1.94728 23.5706 2.24043 23.6918C2.53358 23.8131 2.84777 23.8753 3.165 23.875H8.02125C8.33849 23.8753 8.65267 23.8131 8.94582 23.6918C9.23897 23.5706 9.50533 23.3927 9.72965 23.1684C9.95397 22.9441 10.1318 22.6777 10.2531 22.3846C10.3743 22.0914 10.4366 21.7772 10.4363 21.46V16.6038C10.4339 15.9638 10.1788 15.3508 9.72645 14.8982C9.27408 14.4456 8.66116 14.1901 8.02125 14.1875Z"
                  stroke="#5260FF"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </svg>
            </span>
            <span>Дэшборд</span>
          </a>
        </div>

        <a
          className={`dash-item dash-item--portfolio${isPortfolio ? ' is-active' : ''}`}
          href="/dashboard/specialist/portfolio"
        >
          <span className="dash-icon">
            <svg
              width="27"
              height="24"
              viewBox="0 0 27 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18.25 5.75C18.25 3.3925 18.25 2.215 17.5175 1.4825C16.785 0.75 15.6075 0.75 13.25 0.75C10.8925 0.75 9.715 0.75 8.9825 1.4825C8.25 2.215 8.25 3.3925 8.25 5.75M0.75 14.5C0.75 10.9887 0.75 9.23375 1.5925 7.9725C1.95739 7.42632 2.42632 6.95739 2.9725 6.5925C4.23375 5.75 5.9875 5.75 9.5 5.75H17C20.5113 5.75 22.2663 5.75 23.5275 6.5925C24.0737 6.95739 24.5426 7.42632 24.9075 7.9725C25.75 9.23375 25.75 10.9875 25.75 14.5C25.75 18.0125 25.75 19.7663 24.9075 21.0275C24.5426 21.5737 24.0737 22.0426 23.5275 22.4075C22.2663 23.25 20.5125 23.25 17 23.25H9.5C5.98875 23.25 4.23375 23.25 2.9725 22.4075C2.42632 22.0426 1.95739 21.5737 1.5925 21.0275C0.75 19.7663 0.75 18.0125 0.75 14.5Z"
                stroke="#696E82"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
              <path
                d="M5.75 10.75L6.565 11.0025C10.9222 12.334 15.5778 12.334 19.935 11.0025L20.75 10.75M13.25 12V14.5"
                stroke="#696E82"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
            </svg>
          </span>
          <span>Портфолио</span>
        </a>

        <a
          className={`dash-item dash-item--profile${isProfile ? ' is-active' : ''}`}
          href="/dashboard/specialist/profile"
        >
          <span className="dash-icon">
            <svg
              width="22"
              height="22"
              viewBox="0 0 22 22"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M0.75 18.25C0.75 16.9239 1.27678 15.6521 2.21447 14.7145C3.15215 13.7768 4.42392 13.25 5.75 13.25H15.75C17.0761 13.25 18.3479 13.7768 19.2855 14.7145C20.2232 15.6521 20.75 16.9239 20.75 18.25C20.75 18.913 20.4866 19.5489 20.0178 20.0178C19.5489 20.4866 18.913 20.75 18.25 20.75H3.25C2.58696 20.75 1.95107 20.4866 1.48223 20.0178C1.01339 19.5489 0.75 18.913 0.75 18.25Z"
                stroke="#696E82"
                strokeWidth="1.5"
                strokeLinejoin="round"
              />
              <path
                d="M10.75 8.25C12.8211 8.25 14.5 6.57107 14.5 4.5C14.5 2.42893 12.8211 0.75 10.75 0.75C8.67893 0.75 7 2.42893 7 4.5C7 6.57107 8.67893 8.25 10.75 8.25Z"
                stroke="#696E82"
                strokeWidth="1.5"
              />
            </svg>
          </span>
          <span>Мой профиль</span>
        </a>
      </nav>

      <button className="dash-item logout" type="button" onClick={() => logout()}>
        <span className="dash-icon">
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M15 4.00098H5V18.001C5 18.5314 5.21071 19.0401 5.58579 19.4152C5.96086 19.7903 6.46957 20.001 7 20.001H15M16 15.001L19 12.001M19 12.001L16 9.00098M19 12.001H9"
              stroke="#696E82"
              strokeWidth="1.5"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          </svg>
        </span>
        <span>Выход</span>
      </button>
    </aside>
  )
}
