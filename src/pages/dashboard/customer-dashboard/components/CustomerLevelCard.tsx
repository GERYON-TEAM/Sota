...
export default function CustomerLevelCard({
  status,
  periodValue,
  periodOpen,
  onToggle,
  ...
  return (
    <div className="level-card">
      ...
      <div className="level-card__progress">
        <button
          className={`level-card__period${periodOpen ? ' is-open' : ''}`}
          type="button"
          onClick={onToggle}
        >
        ...
        {periodOpen && (
          <div className="level-card__menu">
            {options.map((item) => {
              const maskId = `mask_level_${item.replace(/\s+/g, '_')}`
              return (
                <button
                  key={item}
                  className="level-card__item"
                  type="button"
